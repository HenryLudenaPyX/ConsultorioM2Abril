...
public class TypeMedicalAttentionRegisterController {
    ...
    public void initialize() {
        ...
        setDataParameters();
        txtAttCode.setOnAction(e -> {
            txtAttName.requestFocus();
            Validations.validateExistenceTypeAttention(txtAttCode, lblAttCode, errors, "type_attention");
        });
        txtAttName.setOnAction(e -> {
            txtAtteInformation.requestFocus();
            Validations.validateNulls(txtAttName, lblAttName, errors);
        });
        txtAtteInformation.setOnAction(e -> {
            txtAttPrice.requestFocus();
            Validations.validateNulls(txtAtteInformation, lblAttInformation, errors);
        });
        txtAttPrice.setOnAction(e -> {
    ...
    void saveRegister(ActionEvent event) {
        ...
        Alert alert;
        String insertType = "INSERT INTO type_attention (id, name, aditionalInformation, unitprice) VALUES (?, ?, ?, ?)";
        Connection connection = DBConnection.connection();
        if (errors.isEmpty()){
            alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setTitle("Mensaje de confirmación");
            alert.setHeaderText(null);
            alert.setContentText("Estás seguro de guardar el registro?");
            Optional<ButtonType> option = alert.showAndWait();
            if (option.get().equals(ButtonType.OK)) {
                try {
                    PreparedStatement preparedStatement = connection.prepareStatement(insertType);
                    preparedStatement.setInt(1, Integer.parseInt(txtAttCode.getText()));
                    preparedStatement.setString(2, txtAttName.getText());
                    preparedStatement.setString(3, txtAtteInformation.getText());
                    preparedStatement.setDouble(4, Double.parseDouble(txtAttPrice.getText()));
                    preparedStatement.executeUpdate();
                    resetTextFields();
                    alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("ÉXITO");
                    alert.setHeaderText(null);
                    alert.setContentText("El tipo de atención médica se ha registrado con éxito.");
                    alert.showAndWait();
                } catch (SQLException e) {
                    System.out.println("Error in register type attention : " + e.getMessage());
                    e.printStackTrace();
                }
            }
        }else{
    ...
    public void setDataParameters(){

    }
    public void resetTextFields(){
        txtAttCode.clear();
        txtAttName.clear();
        txtAtteInformation.clear();
        txtAttPrice.clear();
    }
