...
import com.prototype.cm2a.components.UserCard;
...
public class UserSearchController {
    ...
    private List<User> Cards(){
        ...
        String query = "SELECT idNumber, names, surnames, rol, phone FROM users WHERE status = 'Activo'";
    ...
    private void filterCards(String searchText) {
        ...
        for (User user : Cards) {
            if (user.getIdNumber().toLowerCase().contains(searchText.toLowerCase())) {
                addCardToGridPane(user, column, row);
                column++;
                if (column == columns + 1) {
                    column = 1;
                    row++;
    ...
    private void addCardToGridPane(User card, int column, int row) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader();
            fxmlLoader.setLocation(getClass().getResource("/com/prototype/cm2a/components/user-card.fxml"));
            Pane cardBox = fxmlLoader.load();
            UserCard userCard = fxmlLoader.getController();
            userCard.setDataCard(card);
    ...
    public void initialize() throws IOException {
        ...
        for (int i = 0; i < Cards.size(); i++) {
            ...
            Pane cardBox = fxmlLoader.load();
            UserCard userCard = fxmlLoader.getController();
            userCard.setDataCard(Cards.get(i));
